...
String CommandProcessor::setWeather(ConnectionId connectionId, String const& argumentString) {
  ...
  auto coordinate = CelestialCoordinate(arguments.at(0));
  size_t index = lexicalCast<size_t>(arguments.at(1));

  bool done = m_universe->setWeather(coordinate, index);

  return done ? strf("set weather for {} to index {}", coordinate, index) : "failed to set weather";
}
